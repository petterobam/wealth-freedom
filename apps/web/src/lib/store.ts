...
const DEFAULT_BUDGETS: Budget[] = [
  { id: "b1", category: "餐饮", limit: 2000, period: "monthly", icon: "🍜", color: "#e74c3c", createdAt: "2026-01-01T00:00:00Z" },
  { id: "b2", category: "交通", limit: 500, period: "monthly", icon: "🚇", color: "#3b82f6", createdAt: "2026-01-01T00:00:00Z" },
  { id: "b3", category: "购物", limit: 1500, period: "monthly", icon: "🛍️", color: "#f59e0b", createdAt: "2026-01-01T00:00:00Z" },
  { id: "b4", category: "娱乐", limit: 800, period: "monthly", icon: "🎮", color: "#8b5cf6", createdAt: "2026-01-01T00:00:00Z" },
...
export function addBudget(b: Omit<Budget, "id" | "createdAt">): Budget {
  const data = loadData();
  const newB: Budget = {
    ...b,
  ...
  data.budgets = [...(data.budgets || []), newB];
  save(data);
  return newB;
}

export function updateBudget(id: string, patch: Partial<Omit<Budget, "id" | "createdAt">>): Budget | null {
  const data = loadData();
...
export function deleteBudget(id: string): boolean {
  ...
  data.budgets = budgets.filter((b) => b.id !== id);
  if (data.budgets.length === before) return false;
  save(data);
  return true;
}

export function getBudgetSpent(category: string): number {
  const now = new Date();
  const monthStart = new Date(now.getFullYear(), now.getMonth(), 1).toISOString().slice(0, 10);
  return loadData().transactions
    .filter((t) => t.type === "expense" && t.category === category && t.date >= monthStart)
